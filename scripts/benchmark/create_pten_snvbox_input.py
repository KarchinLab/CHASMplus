...
Github: ctokheim
Description: Create transcript input format for snvbox for pten saturation mutagenesis data
"""
import pandas as pd
import argparse
import os
...
def main(opts):
    # read iarc mutations
    df = pd.read_table(opts['pten'])
    df['mutation'] = df['Variant (one letter)']
